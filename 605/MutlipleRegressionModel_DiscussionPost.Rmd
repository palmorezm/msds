...
```{r}
set.seed(42)
angle <- sample(1:99, 500, replace = T)
ev <- sample(0:119, 500, replace = T)
pheight <- runif(500, 5.8, 6.7)
hits <- data.frame(cbind(angle, ev, pheight)) 
hits <- hits %>% 
  mutate(rad = angle * (pi/180)) %>% 
  mutate(ms = ev / 2.237) %>% 
  mutate(m = pheight / 3.281 ) %>%
  mutate(g = 9.81) %>% 
  mutate(dist = ms * cos(rad) * (ms * sin(rad) + sqrt((ms * sin(rad))^2 + 2 * g * m)) / g) %>% 
  mutate(xHR = as.numeric(dist > 99)) %>% 
  mutate(HR = as.numeric(dist >= 99.06 & mph > 95) )
hits[c(1:3, 8:10)]
hits <- hits[c(1:3,8:10)] %>% 
  filter(mph > 40) 
hits %>% 
...
g <- 9.81
v * cos(a) * v * sin(a) 
(sqrt(((v * sin(a))^2 + 2 * g * h)/ g))
v * cos(a) * v * sin(a) + (sqrt((((v * sin(a))^2) + (2 * g * h)))/g)
v * cos(a) * abs(v * sin(a) + (sqrt((((v * sin(a))^2) + (2 * g * h)))/g))
v * cos(a) * (v * sin(a) + sqrt((v * sin(a))^2 + 2 * g * h)) / g # Correct
```
