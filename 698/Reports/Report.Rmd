...
Instead, focus should be placed on direct consumer access to housing without loan considerations and incorporate a reasonable safety net in these index calculations. We propose a new method of calculating housing affordability for consumers in the 21st century, one that incorporates a set of minimum thresholds that better exemplify the economic challenges of the modern home buying process. 


Our hypothesis is that improving upon the housing affordability index will increase understanding among financial institutions and municipalities to empower prospective homebuyers in their areas. 

To elucidate upon the effects of each HAI method we model their distributions across income and population by MSA. 

estimate the proportion of the population that is unable to afford housing

We also compare rates of change in personal and real wage income to the change in housing affordability at various HAI levels

Use linear regression techniques to approximate how many people could afford housing in the future under each HAI measurement. 


Estimates based on the NAR method found that in 2019 approximately [x] people would be unable to afford housing in the MSAs of the U.S. We developed 6 alternative methods to better exemplify the modern home-buying process, each with a focus on a particular interest factor or subset of variables that are currently understood to influence the housing market. Our results were tabulated and are shown alongside the national standard denoted with an asterisk for comparison: 


The Outstanding Debts HAI method stands out the most with no person being able to afford housing. This indicates that if the debts of the average household were required to be paid immediately prior to the purchase of a house, no household in the U.S. would be able to afford the home with the current average amount of household debt. Given this HAI only considers consumer debts, which does not include mortgages, the value represents a shift in household finances towards greater debts for its goods and services even as lending practices have tightened since the 2008 financial crisis. 

There are multiple ways to interpret the results of our HAI metrics and how well they perform. What is realistic to someone who has already purchased a house and is maintaining a long-term wealth assets, will not be as realistic to someone who has been renting their whole life. Rather than make this analysis is a host of situation specific scenarios, we draw on deviations from our current national average to make interpretations and pass judgement about its effects on the economy and people. This holistic approach is best represented by the following scatterplot of median income per HAI level where the dotted vertical line represents the current nationally accepted HAI value in which homes could be afforded. The colored points surrounding this line correspond to real MSA HAI values in the U.S. while a LOESS smoothing line is provided for reference. 
...
On again, the Outstanding Debts HAI is observed farthest from affordable and as such must be displayed on a separate scale. Recall that we also increased lending-leniency to allow for 30% of monthly personal income to be spent on housing to try and compensate for the additional debts. This is consistent with the maximum possible loan amounts (relative to household income) commonly granted by 2019 financial standards yet, it marginally improves results. 

From this plot we use two visuals fully comprehend the change in median income as affordability increases of this Debt-laiden method. On the bottom left we see the change in median income per MSA at the same scale of the other HAI methods. In it, the rate of change is nearly vertical. On the bottom right, we see a smoother linear transition from lower, negative HAI values to positive ones. However, there is a slight uptick at the elbow of this plot located at an HAI of about 1. It is also clear that this is the only HAI method that resulted in value below zero.  

Although the average household has not always been as debt-heavy, it is unrealistic to expect that consumers interested in purchasing a house would first pay off all their debts. Credit has existed in several forms since the 1900's but its prolific use is still a relatively new phenomena. We should not expect that its proliferation will cease anytime soon. Out interests are to inform the public and policymakers alike. For this reason, it will be discarded in favor of other methods.  

With our the remaining methods located at the top of the plot, we can inspect the proportions of MSA that occur above and below the national average. At first glance, the Lenient Lending HAI pulls nearly all MSA values into the 'affordable' section above the national average. This is followed by the Inflation Adjusted HAI which assumed the median household value of each MSA maintained its present value. The remaining methods are too close to call so we revist them with another visual. 

## Stoped Here - Continue Below -->

The determining factors of which of these HAI plots performs best is dependent upon which factors are weighted most heavily. For example, if affordability is the key and there is little care about market stability, the lenient lending HAI is best suited to measure relative affordability. This is because it has by far the greatest proportion of MSAs located at a higher HAI value than the current national average located at an HAI of 100. By the same notion, if affordability is solely dependent upon how many of these MSA can be on the right side of the vertical line denoting the national average HAI, then the Outstanding Debts HAI would portray the least affordable HAI of all as no MSA is even close to the national average. Many are below zero making it impractical at best for those MSAs to afford anything, let alone housing, regardless of Median Income.     
...
df.fin %>% 
  ...
  filter(key == c("HAI", "HAIRW", 
                  ...
                  "HAILEN", 
                  "HAIDBT")) %>%
  summarise(sum(is.na(value))) %>% View()
# Extract HAIs < 100
ltdf <- df.fin %>% 
  gather(key, value, -GeoFips, -GeoName, 
         -year, -MEDINC, 
...
# sum(gttbl$N) 
# sum(lttbl$N)
df <- rbind(lttbl, gttbl)
head(df)
```


```{r}
df$year <- as.Date(paste(df$year, 12, 31, sep = "-"))
df %>% 
  group_by(key, Set) %>% 
...
```{r}
RMEDINC.median <- median(df$RMEDINC)
df %>% 
  group_by(key, Set) %>% 
  ggplot(aes(x = year, col = key)) + geom_point(aes(y = RMEDINC, col = key)) + 
  geom_hline(yintercept = RMEDINC.median, lty = "dotted") + 
  geom_line(aes(y = RMEDINC)) +
  facet_wrap(~Set, scales = "free") +
  geom_smooth(aes(y = RMEDINC), 
              method="loess",
              col = "grey30", 
              fill = "light grey", 
              se = T, na.rm = T, 
              lty = "solid")
RMEDINC_reduced <- (df$RMEDINC / 1000)
RMEDINC_reduced.median <- median(RMEDINC_reduced)
df %>% 
  group_by(key, Set) %>% 
  ggplot(aes(x = year, col = key)) + geom_point(aes(y = RMEDINC_reduced, col = key)) + 
  geom_hline(yintercept = RMEDINC_reduced.median, lty = "dotted")  +
  facet_wrap(~key) +
  geom_smooth(aes(y = RMEDINC_reduced), 
              method="loess",
              col = "grey30", 
              fill = "light grey", 
              se = T, na.rm = T, 
...
df %>% 
  filter(key != "HAIDBT") %>%
  group_by(key, Set) %>% 
  ggplot(aes(x = MMEDHAI, col = key)) + geom_point(aes(y = RMEDINC, col = key)) + 
  geom_hline(yintercept = RMEDINC.median, lty = "dotted") + 
  geom_point(aes(y = RMEDINC)) +
  facet_wrap(~Set, scales = "free") +
  geom_smooth(aes(y = RMEDINC), 
              method="loess",
              col = "grey30", 
...
df.fin %>% 
  gather(key, value, -GeoFips, -GeoName, -year, -MEDINC, 
         -MEDVAL, -MOE, -UMEDVAL, -LMEDVAL, -POP, -PERINC, -AINCALL) %>%
  filter(key == c("HAI", "HAIRW", "HAIRNT", "HAIDBT",
                  "HAIIPD", "HAIRAW", "HAILEN")) %>% 
  filter(AINCALL <100000) %>% 
  ggplot(aes(value, col = key)) + 
...
Create a table of the above values with a tally of how many are above or below average. Determine which of these statistics is most realistic for HAI purposes and when estimating home affordability across the U.S. 

We believe that each has a unique perspective to offer. For purposes of former generations, lenient lending appears to have made it a lot easier to afford homes but at the cost of market stability (given the great depression, recession, and so on). The raw values also show that if practices were the same before the FHA was created the most amount of people would not be able to afford a home (excluding debts). When including the debts of the average homeowner today, there is no place in the U.S. deemed affordable by NAR HAI standards. Additionally, if incomes were adjusted for prospective inflation (and somehow the price of housing were to stagnate), a large majority of the metropolitan areas in the US would be affordable. Real wage estimates and Rent Adjusted estimates capture the minimum realities that most housholds face; the aggregated cost of a goods and services and the effect rent can have on encouraging (or in this case discouraging) homeownship in the U.S. Since rent is practically required of all potential homeowners (look for sources on this), it could be considered the most useful HAI metric from this analysis. 

The goal should be to provide the most people with housing at an affordable cost without burdening the market or other individuals to achieve it. This should include sustainability goals which are not covered in this analysis.  
## Discussion 
