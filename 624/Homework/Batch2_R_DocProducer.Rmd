...
Those selected are shown in the model summary which had a multiple R2 value of 0.6689 indicating the model captures roughly 67% of the variation in response. We note that, this is likely not the best model for prediction with this data and the only significant predictor (based on an alpha level of 0.05) is ManufacturingProcess32. However, this was a simple model choice we could gather information from and improve on as necessary since we were given the option to tune the model of our choice.
