...
#### Response
After splitting the data 75% training and 25% test datasets we chose to build a linear regression model using the top 7 features that had the strongest correlations with ‘Yield.’  They were selected based on absolute correlation strength. A table of all variable correlations with Yield is shown for reference. 
...
Those selected are shown in the model summary which had a multiple R2 value of 0.6395 indicating the model captures roughly 64% of the variation in response. This is likely not the best model for prediction with this data and the only significant predictor (based on an alpha level of 0.05) is ManufacturingProcess32. However, this was a simple model choice we could gather information from and improve on as necessary since we were given the option to tune the model of our choice.
