...
```{r}
# I would use the same command but reference the 'girls' column of the data frame like this
arbuthnot$girls
```
...
I would say there is a positive trend in the number of girls baptized over the years from 1629 to 1710 based on visual analysis of a linear line of best fit. It shows a general positive trend when considering all baptized girls over the entire study duration. This can be shown here:
   
```{r}
# Recreating the scatterplot of the number of girls baptized per year
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_point() +
# Creating a linear line of best fit using all points in the study
...
It appears as though more boys being born over time but that trend is decreasing.

When plotting the ratio of boys over time and using a linear line of best fit, a slightly negative slope indicates that there are fewer boys being born over the duration of this study. However, the ratio of boys to the total number of births is still greater than 0.50 indicating a higher number of boys being born. 
...
```{r}
# tested first to see if present was built in by running 'present'
# then pulled it into the data frame called 'present'
present <- present
```
...
Years included in this data frame are from 1940 to 2002. There are 63 rows and 3 columns in the data frame. The first column is automatically generated by R and is not considered part of the data frame in the following *glimpse* function used to calculate the number of rows and columns. The variables or column names are "year", "boys", and "girls". 
...
These counts are not of similar magnitude. They are much larger than Arbuthnot's counts. In fact, the lowest count of boys in this data set is over 419 times larger than Arbuthnot's lowest count of boys in the first year of both studies. This can be shown by dividing the count of boys in present's data frame for the year 1940 by the minimum of Arbuthnot's count of boys born in 1629. 

```{r}
min(arbuthnot$boys)
# min of arbuthnots boys count is 2,890
min(present$boys)
# min of present boys count is 1,211,684
magnitude_boys <- (min(present$boys))/(min(arbuthnot$boys))
```
...
I will be reusing the code from exercise 3 to compute multiple new columns. One where the number of boys born is divided by the number of girls born to create a ratio of boys to girls born in each year. Another with the sum of boys and girls born to each year. This new column name will be called "total." Then lastly, the ratio of boys born to the total births. It will be called "boy_ratio".

```{r}
# create a new column of the ratio of boys born to girls born
present <- present %>%
  mutate(boys_to_girls_ratio = boys / girls)
# create a new column of the sum of boys and girls called total
present <- present %>%
  mutate(total = boys + girls)
# create a new column of the ratio of boys born to the total births 
present <- present %>% 
  mutate(boy_ratio = boys / total)
```

Here is a plot that displays the proportion of boys to girls born over time. when compared to the total births in each year for the present data frame. 
    
```{r}
ggplot(data = present, aes(x = year, y = boys_to_girls_ratio)) +
  geom_point() + 
  geom_smooth(method = lm)
```

In this case, there are still more boys being born than girls. While the ratio is only slightly skewed towards boys at this magnitude there a lot more boys. For example in 1940 and 2002 there were 62,969 and 94,232 more boys born than girls.

```{r}
present <- present %>%
  mutate(difference_boys_to_girls = boys - girls)
```

A new column was also created to show the difference in boys and girls born at each year.
...
The year with the most total number of births in the U.S. 1961 with 4,268,326 total births. This was found using the following method;

A column with the sum of boys and girls has already been created above for fun. It is called 'total'. Using the sample code provided the data will be arranged in descending order in the data frame.  

```{r}
# Arrange in descending order and store it the data frame
present <- present %>%
  arrange(desc(total))
```

To find the year with the most total number of births in the U.S. I will first use the max function. This will tell me what the highest value is and then I can use the which function to locate the row with the max value in the data frame. I will then pull that row's year out of the data frame.

```{r}
# finding the most total number of births in the U.S. 
max(present$total)
# find the row with the most total number of births
which(present$total == (max(present$total)))
# displaing the year where the max number of births occured in the U.S. 
present[1, 1]
```

Of course, the data were sorted in descending order prior to my use of the which function. For this reason  I know it it is correct since that is the location I should have found the most total number of births in the U.S. Otherwise, something may have gotten mixed up. Glad to see that was not the case this time. 
