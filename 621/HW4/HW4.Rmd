...
```{r}
tdata %>% 
  select("TARGET_FLAG", "MVR_PTS", "CLM_FREQ", "HOMEKIDS", "KIDSDRIV", "TIF") %>% 
  gather() %>% 
  ggplot(aes(value)) +
  facet_wrap(~ key, scales = "free") + 
...
chars <- select(tdata, where(is.character))
...
as.data.frame(cbind(income.values, 
                    home.values, 
                    bluebook.values, 
                    oldclaim.values))
...
summary(dollar.values)
summary(dollar.values.stats)
```
